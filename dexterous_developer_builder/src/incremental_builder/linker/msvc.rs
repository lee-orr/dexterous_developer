...
use super::super::builder::IncrementalRunParams;
use anyhow::{bail, Context};
use camino::Utf8PathBuf;
use futures_util::future::join_all;
use tokio::io::AsyncWriteExt;

pub async fn msvc_linker(args: Vec<String>, package_name: String, output_file: String, target: String, lib_directories: Vec<Utf8PathBuf>) -> anyhow::Result<()> {
    let linker_exec = "rust-lld.exe".to_string();
    ...
    let output_name = {
        args.iter()
            ...
            .map(|v| v.trim_start_matches("/OUT:").to_owned())
            .unwrap_or_default()
    };


    args.insert(0, "link".to_owned());
    args.insert(0, "-flavor".to_owned());
    panic!("Args for Linking\n{}", args.join("\n"));

    if !output_name.contains(&package_name) {
        eprintln!("Linking Non-Main File - {output_name}\n{}", args.join(" "));

        let mut cmd = tokio::process::Command::new(linker_exec);
        cmd.args(args.clone());
...
async fn adjust_arguments(
    ...
    for dir in lib_directories.iter().rev() {
        new_args.push(format!("/LIBPATH:{dir}"));
    }
